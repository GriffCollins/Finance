...
"""
Volatility Expansion Close
It looks for price movements beyond channels defined via the Average True Range
Channels are so accurate that it is unlikely to go outside them
Reverse variable makes the strategy mean reversal, strategy is normally breakout 
It appears that this strategy will work better when it can dynamically switch between both for regime changes
"""
...
df['UB'] = df['Close'] + multiplier * df['ATR']
df['LB'] = df['Close'] - multiplier * df['ATR']

if reverse:
    df.loc[df['Close'] > df['UB'].shift(1), 'signal'] = -1
    df.loc[df['Close'] < df['LB'].shift(1), 'signal'] = 1
else:
    df.loc[df['Close'] > df['UB'].shift(1), 'signal'] = 1
    df.loc[df['Close'] < df['LB'].shift(1), 'signal'] = -1
...
#Plot volatility expansion channel
plt.figure(figsize=(24, 6))
plt.plot(df.index, df['Close'], label='Close', linewidth=1, color='C0')
plt.plot(df.index, df['UB'], label='UB', linewidth=1)
plt.plot(df.index, df['LB'], label='LB', linewidth=1)
plt.ylabel('Momentum Indicator')
plt.xlabel('Date')
plt.legend()
plt.title(f'{ticker} Volatility Expansion Close Time Series')
...
plt.xlabel('Date')
plt.legend()
plt.title(f'{ticker} Volatility Expansion + Close Strategy')
plt.grid(True, alpha=0.3)
plt.show()

#Create dataframe csv
df.to_csv('Volatility Expansion Close Trader.csv')