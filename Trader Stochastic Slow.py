...
# Initial parameters
ticker = 'SPY'
N = 5 #Range of finding highest and lowest high and low prices
length = 3 #Smoothing period
...
df['High_nMax'] = df['High'].rolling(N, min_periods=1).max(axis=1)
df['Low_nMin'] = df['Low'].rolling(N, min_periods=1).min()
df['%K'] = (df['Close'] - df['Low_nMin']) / (df['High_nMax'] - df['Low_nMin']) * 100
df['%D'] = df['%K'].rolling(length, min_periods=1).mean()
df['%D_Slow'] = df['%D'].rolling(length, min_periods=1).mean()

if reverse:
    df.loc[(df['%D'] < df['%D_Slow']) & (df['%D'].shift(1) >= df['%D_Slow'].shift(1)) & (df['%D'] > 80), 'signal'] = 1
    df.loc[(df['%D'] > df['%D_Slow']) & (df['%D'].shift(1) <= df['%D_Slow'].shift(1)) & (df['%D'] < 20), 'signal'] = -1
else:
    df.loc[(df['%D'] < df['%D_Slow']) & (df['%D'].shift(1) >= df['%D_Slow'].shift(1)) & (df['%D'] > 80), 'signal'] = -1
    df.loc[(df['%D'] > df['%D_Slow']) & (df['%D'].shift(1) <= df['%D_Slow'].shift(1)) & (df['%D'] < 20), 'signal'] = 1
...
plt.plot(df.index, df['%D'], label='%D', linewidth=1, color='C0')
plt.plot(df.index, df['%D_Slow'], label='%D_Slow', linewidth=1)
plt.ylabel('Momentum Indicator')
plt.xlabel('Date')
plt.legend()
plt.title(f'{ticker} Stochastic Slow Momentum Indicator')
...
plt.xlabel('Date')
plt.legend()
plt.title(f'{ticker} Stochastic Slow Strategy')
plt.grid(True, alpha=0.3)
plt.show()

#Create dataframe csv
df.to_csv('Stochastic Slow Trader.csv')