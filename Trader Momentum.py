...
Long stocks when standardised momentum is above x sigma
Short stocks when standardised momentum is below x sigma
Includes contrarian signal generator
Make sure yfinance limits are within 2 months for intraday data
"""
...
reverse = True #Standard = False, Contrarian = True
...
df['moving_momentum'] = (length + 1) * (df['moving_average'] - df['moving_average'].shift(1))
df['z-momentum'] = df['moving_momentum'] / df['moving_std']

if reverse:
    df.loc[df['z-momentum'] > 1, 'signal'] = -1
    df.loc[df['z-momentum'] < -1, 'signal'] = 1
else:
    df.loc[df['z-momentum'] > 1, 'signal'] = 1
    df.loc[df['z-momentum'] < -1, 'signal'] = -1
...
#Plot standardised momentum
plt.figure(figsize=(12, 6))
plt.step(df.index, df['z-momentum'], label='Standardised momentum', linewidth=1)
plt.ylabel('Z-Score')
plt.xlabel('Date')
plt.legend()
plt.title(f'{ticker} Momentum Strategy Z-Score Time Series')
